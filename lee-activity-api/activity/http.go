...
func ListAllRunningActivity() ([]Activity, error) {
	// TODO：环境判断
	uri := "http://127.0.0.1:8080/api/activity/config/list_all_running_activity"
	req := &ListAllRunningActivityReq{}

	resp := new(base.Resp)
	err := base.HttpPost(uri, req, &resp)
	if err != nil {
		logger.Errorf("ListAllRunningActivity error:%v", err)
		return nil, err
	}
	if !resp.IsSuccess() {
	...
	// TODO：优化解析
	var data ListAllRunningActivityResp
	b, _ := json.Marshal(resp.Data)
	_ = json.Unmarshal(b, &data)
	return data.AllRunningActivityList, nil
}
